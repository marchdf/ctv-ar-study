...
# Solve for the stretching factor, given the aspect ratio and assuming
...
proc = sp.Popen(
    spackbin + " location -i openmpi ", shell=True, stdout=sp.PIPE, stderr=sp.PIPE
)
...
proc = sp.Popen(
    mpibin + " -np 1 " + abl_mesh + " -i " + msh_oname, shell=True, stderr=sp.PIPE
)
